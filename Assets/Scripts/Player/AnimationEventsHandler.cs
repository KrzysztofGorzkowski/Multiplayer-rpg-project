...
public class AnimationEventsHandler : MonoBehaviour
{
    ...
    {
        GameManager.GetPlayer().mobilitySwitch(); //unlocks player movement
        GameManager.GetLabirynth().SwapTeleportStartTile(); //change start teleport's tile into OFF mode
    }

    //called during shooting animations, when the real shot has to be done
    public void Shoot()
    {
        GameManager.GetPlayer().getPlayerObject().GetComponent<Shooting>().Shoot();
    }
